...
function PagesGrid({ time }: PagesGridProps) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      setLoading(true);
      const res = await api.get(`/metrics/${curSite.id}/pages/list`, {
        params: {
    ...
    <div className={styles.container}>
      <Spin spinning={loading}>
        <div className={styles.titlebar}>
          <span className={styles.title}>受访页</span>
          <Link to='/pages'>详情</Link>
        </div>
...
export default observer(PagesGrid);
